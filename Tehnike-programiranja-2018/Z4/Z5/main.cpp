...
class Administracija
{
  ...
  Administracija(const Administracija &admin)
  {
    ...
    {
      mapa_studenata[ita->first] = new Student(*ita->second);
    }
  ...
  void RegistrirajNovogStudenta(int br_indexa, std::string g_studija, std::string ime_prezime, std::string adr, std::string broj_telefona)
  {
    ...
    mapa_studenata[pomocni.DajIndeks()] = new Student(pomocni);
  ...
  void RegistrirajNoviLaptop(int e_broj, std::string nejm, std::string kerek)
  {
    try
    {
        //Laptop pomocni(e_broj, nejm, kerek);
        ...
        mapa_laptopa[e_broj] = new Laptop(e_broj, nejm, kerek);
    }catch(std::domain_error){throw;}
  }
...
{
  try{
    Student s1(12345, "1/B", "Neko nekic", "bascarsija", "111");
    std::cout<<"ovo se ne smije ispisat";
  }catch(std::domain_error e){std::cout<<e.what();}
  catch(...){std::cout<<"kontra izuzetak";}
