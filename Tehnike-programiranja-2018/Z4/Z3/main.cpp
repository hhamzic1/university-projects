...
class Pregledi
{
  ...
  Pregled **pregledi;
  public:
    explicit Pregledi(int max_broj_pregleda): max_br_pregleda(max_broj_pregleda), pregledi(new Pregled*[max_broj_pregleda]){broj_pregleda=0;}
    Pregledi(std::initializer_list<Pregled> spisak_pregleda):max_br_pregleda(spisak_pregleda.size()), pregledi(new Pregled*[spisak_pregleda.size()])
    {
        auto it(spisak_pregleda.begin());
        for(int i=0; i<spisak_pregleda.size(); i++)
        {
            pregledi[i]=new Pregled(it->DajImePacijenta(), it->DajDatumPregleda(), it->DajVrijemePregleda());
            it++;
        }
    ...
    Pregledi(const Pregledi &p): max_br_pregleda(p.max_br_pregleda), pregledi(new Pregled*[p.max_br_pregleda])
    {
    ...
    Pregledi(Pregledi &&p): max_br_pregleda(p.max_br_pregleda), pregledi(new Pregled*[p.broj_pregleda])
    {
        for(int i=0; i<p.broj_pregleda; i++)
        {
            pregledi[i]=new Pregled(*(p.pregledi[i]));
    ...
    Pregledi &operator =(const Pregledi &p)
    {
        ...
        pregledi=new Pregled*[p.broj_pregleda];
        for(int i=0; i<p.broj_pregleda; i++)
        {
            pregledi[i]=new Pregled(*(p.pregledi[i]));
        }
    ...
    void RegistrirajPregled(Pregled *p)
    {
        if(broj_pregleda+1>max_br_pregleda) throw std::range_error("Dostignut maksimalni broj pregleda");
        try{
            pregledi[broj_pregleda]=new Pregled((*p).DajImePacijenta(), (*p).DajDatumPregleda(), (*p).DajVrijemePregleda());
        }catch(std::range_error){throw;}
        broj_pregleda++;
    }
    ...
    {
        return std::count_if(pregledi, pregledi+broj_pregleda, [datum](Pregled* p1){auto k=(*p1).DajDatumPregleda(); auto prvi=k.Ocitaj(); auto drugi=datum.Ocitaj();
        return prvi==drugi;});
    }
    Pregled &DajNajranijiPregled()
    {
        if(broj_pregleda==0) throw std::domain_error("Nema registriranih pregleda");
        return **std::min_element(pregledi, pregledi+broj_pregleda, [](Pregled *x, Pregled *y){
            return Pregled::DolaziPrije(*x,*y);
        });
    }
    ...
    void ObrisiNajranijiPregled()
    {
        ...
        auto it=*std::min_element(pregledi, pregledi+broj_pregleda, [](Pregled* p1, Pregled* p2){return Pregled::DolaziPrije(*p1,*p2);});
        for(int i=0; i<broj_pregleda; i++)
        {
            if(pregledi[i]==it)
            {
    ...
    void ObrisiPregledePacijenta(const std::string &ime_pacijenta)
    {
        for(int i=0; i<broj_pregleda; i++)
        {
            auto index=Pregledi::pronadjiga(ime_pacijenta, broj_pregleda);
            if(i==index)
            {
...
int main ()
{
    ...
    for(int i=0; i<n; i++)
    {
        ...
        catch(std::domain_error e){std::cout<<e.what()<<std::endl; i--; continue;}
        ...
        }catch(std::domain_error v){std::cout<<v.what()<<std::endl; i--; continue;}
        try{
        proba.RegistrirajPregled(pomocni, dej, mant, jr, klok, min);
        }catch(std::range_error e){std::cout<<e.what()<<std::endl; i--;}
        catch(std::domain_error a){std::cout<<a.what()<<std::endl; i--;}
    }
        ...
        try
        {
            switch(x)
            {
                ...
                case 3: 
                {
                    ...
                    std::cout<<"Unesite ime pacijenta: ";                           //MORAŠ DODAT TRY KEČEVE ZA SVE ŽIVO ŠTO SE BACA
                    std::string nejm; 
